...
class visualizer(VisualizerAbstractClass):
    ...
    def get_epoch_plot_measures(self, epoch):
        ...
        tar_tensor = torch.Tensor(tar_data).to(self.device)
    ...
    def get_epoch_decision_view(self, epoch, resolution, xy_limit=None):
        ...
        ref_reconstructed = self.transform_model.decoder(torch.tensor(grid_samples).to(self.device)).cpu().detach().numpy()

        mesh_preds = self.tar_data_provider.get_pred(epoch, ref_reconstructed)
    ...
    def savefig(self, epoch, path="vis"):
        ...
        #### use clean labels
        with open(os.path.join(self.tar_data_provider.content_path, 'clean_label.json'), "r") as f:
