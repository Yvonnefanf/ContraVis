...
def orthogonal_mapping(data1, data2):
    ...
    """
    use Orthogonal Mapping map data2 to data1 space。
...
class visualizer(VisualizerAbstractClass):
    def __init__(self, ref_data_provider, tar_data_provider, projector, resolution, indicates, cmap='tab10'):
      
        self.ref_data_provider = ref_data_provider
        self.tar_data_provider = tar_data_provider
        self.projector = projector
        self.cmap = plt.get_cmap(cmap)
        self.classes = ref_data_provider.classes
    ...
    def get_epoch_plot_measures(self, epoch):
        ...
        data = self.tar_data_provider.train_representation(epoch)
        data = orthogonal_mapping(self.ref_data_provider.train_representation(epoch), data)
    ...
    def get_epoch_decision_view(self, epoch, resolution):
        ...
        #TODO
        ### transform ref to tar
        new_grid = orthogonal_mapping(self.tar_data_provider.train_representation(epoch)[:40000], np_grid_samples)


        mesh_preds = self.tar_data_provider.get_pred(epoch, new_grid)
    ...
    def savefig(self, epoch, path="vis"):
        ...
        train_data = self.tar_data_provider.train_representation(epoch)
        train_labels = self.tar_data_provider.train_labels(epoch)
        after_trans = orthogonal_mapping(self.ref_data_provider.train_representation(epoch), train_data)
        if len(self.indicates):
            ...
            after_trans = orthogonal_mapping(self.ref_data_provider.train_representation(epoch), train_data)
        pred = self.tar_data_provider.get_pred(epoch, train_data)
